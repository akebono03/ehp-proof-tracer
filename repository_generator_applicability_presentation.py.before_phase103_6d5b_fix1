...
def _rule_family_relevance_category(
  rule_family,
) -> RuleRelevanceCategory:
  categories = {
    entry.relevance_category
    for entry in rule_family.catalog_entries
  }
...
def _rule_family_relevance_sort_key(
  rule_family,
) -> int:
  category = (
    _rule_family_relevance_category(
      rule_family
    )
  )

  return (
    _RELEVANCE_PRESENTATION_ORDER.index(
      category
    )
  )
  ...
  def rule_families(
    ...
    return tuple(
      sorted(
        rule_families,
        key=(
          _rule_family_relevance_sort_key
        ),
      )
    )
