...
def _is_symbolic_zero_group_step(
  step: ProofStep,
  *,
  stem: int,
) -> bool:
...
def _is_symbolic_prop58_four_stem_zero_step(
  step: ProofStep,
) -> bool:
  return _is_symbolic_zero_group_step(
    step,
    stem=4,
  )


def _is_symbolic_prop59_five_stem_zero_step(
  step: ProofStep,
) -> bool:
  return _is_symbolic_zero_group_step(
    step,
...
def _is_symbolic_prop511_nu_squared_group_step(
  step: ProofStep,
) -> bool:
  if not _is_symbolic_group_relation(
    step,
...
def specialize_toda_prop58_four_stem_zero_index_step(
  symbolic_step: ProofStep,
  index: int,
) -> ProofStep:
  if not isinstance(
  ...
  if not _is_symbolic_prop58_four_stem_zero_step(
    symbolic_step
  ):
    raise ValueError(
      "symbolic_step must prove the symbolic "
      "Toda Proposition 5.8 four-stem zero group"
    )

  return ProofStep(
    conclusion=TodaPrimaryGroupZeroStatement(
      group=TodaPrimaryGroup(
        group_dimension=index + 4,
        sphere_dimension=index,
      ),
    ),
    premises=(
    ...
    rule=ProofRule.INFERENCE,
    note=(
      "Phase 130-8 theorem-specific concrete "
      "specialization of Toda Proposition 5.8 "
      f"at n={index}"
    ),
  )


def specialize_toda_prop59_five_stem_zero_index_step(
  symbolic_step: ProofStep,
  index: int,
) -> ProofStep:
  if not isinstance(
  ...
  if index < 7:
    raise ValueError(
      "index must be at least 7"
    )

  if not _is_symbolic_prop59_five_stem_zero_step(
    symbolic_step
  ):
    raise ValueError(
      "symbolic_step must prove the symbolic "
      "Toda Proposition 5.9 five-stem zero group"
    )

  return ProofStep(
    conclusion=TodaPrimaryGroupZeroStatement(
      group=TodaPrimaryGroup(
        group_dimension=index + 5,
        sphere_dimension=index,
      ),
    ),
    premises=(
    ...
    note=(
      "Phase 130-8 theorem-specific concrete "
      "specialization of Toda Proposition 5.9 "
      f"at n={index}"
    ),
  )


def specialize_toda_prop511_nu_squared_index_step(
  symbolic_step: ProofStep,
  index: int,
) -> ProofStep:
  if not isinstance(
  ...
  if index < 9:
    raise ValueError(
      "index must be at least 9"
    )

  if not _is_symbolic_prop511_nu_squared_group_step(
    symbolic_step
  ):
    raise ValueError(
      "symbolic_step must prove the symbolic "
      "Toda Proposition 5.11 nu-squared group"
    )
  ...
  return ProofStep(
    ...
    note=(
      "Phase 130-8 theorem-specific concrete "
      "specialization of Toda Proposition 5.11 "
      f"at n={index}"
    ),
  )
...
def specialize_repository_proof_scope_for_toda_group_query(
  ...
  if (
    ...
    predicate = (
      _is_symbolic_prop58_four_stem_zero_step
    )
    specialize = (
      specialize_toda_prop58_four_stem_zero_index_step
    )
  elif (
    query.k == 5
    and query.n >= 7
    ...
    predicate = (
      _is_symbolic_prop59_five_stem_zero_step
    )
    specialize = (
      specialize_toda_prop59_five_stem_zero_index_step
    )
  elif (
    query.k == 6
    and query.n >= 9
    ...
    predicate = (
      _is_symbolic_prop511_nu_squared_group_step
    )
    specialize = (
      specialize_toda_prop511_nu_squared_index_step
    )
  else:
    return scope
