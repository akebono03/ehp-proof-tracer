...
def make_phase95_8_source_entry(
  conclusion,
):
  return ProofRepositoryEntry(
    key="phase95.aggregate",
    step=ProofStep(
...
def test_phase95_8_source_preserves_entry_identity_and_branch_name():
  source_entry = (
    make_phase95_8_source_entry(
...
def test_phase95_8_candidate_preserves_source_identity():
  query = TodaGroupQuery(
    n=4,
    k=6,
  )
...
def test_phase95_8_source_rejects_non_repository_entry():
  with pytest.raises(
    TypeError,
    match=(
    ...
    TodaCalculationGoalSource(
      source_entry="not-an-entry",
      branch_name="pi10_4_group_relation",
    )


def test_phase95_8_source_rejects_non_string_branch_name():
  source_entry = (
    make_phase95_8_source_entry(
      conclusion="aggregate conclusion",
    )
  ...
  with pytest.raises(
    TypeError,
    match="branch_name must be a str",
  ):
    TodaCalculationGoalSource(
      source_entry=source_entry,
      branch_name=10,
...
def test_phase95_8_source_rejects_empty_branch_name():
  source_entry = (
    make_phase95_8_source_entry(
      conclusion="aggregate conclusion",
    )
...
def test_phase95_8_candidate_rejects_invalid_source_type():
  query = TodaGroupQuery(
    n=4,
    k=6,
  )
