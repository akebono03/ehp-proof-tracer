...
ast.parse(text)
print("75AP R1/R2 import corruption repaired.")
...
if missing:
    ...
    closing = imp.end_lineno - 1
    if lines[closing].strip() != ")":
        raise RuntimeError("Unexpected toda_rules import ending.")
    insertion = "".join(fixed_lines[name] for name in missing)
    lines.insert(closing, insertion)
    text = "".join(lines)

ast.parse(text)
print("75AP R3 imports applied with real newlines.")
...
function_text = chr(10).join(
    text.splitlines()[func.lineno - 1:func.end_lineno]
)

if "TodaLemma510OrdinaryIndeterminacyDoubleStatement" not in function_text:
    lines = text.splitlines(keepends=True)
...
print("Phase 143-75AP R3 final bulk patch applied.")
