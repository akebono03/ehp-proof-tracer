...
The default depth remains one direct premise level. `--depth 0` shows only the replay root, and larger nonnegative depths expose deeper existing ancestry.

Unsupported deep aggregate statements are rendered by safe type-name fallback instead of raw dataclass representations.

## Existing operation-fact query

The operation-query path is a read-only lookup over already represented repository / proof-scope facts.
...
The flat three-term composition form is interpreted using the existing right-nested production structure. For example,

```text
eta_2 o nu_prime o eta_6
```

matches the represented composition
...
The operation-query layer is intentionally a lookup:
...
Three-term operands inside `E(...)`, `H(...)`, or `Delta(...)`, four-or-more-term composition, parentheses, Unicode `∘`, and a general expression parser remain intentionally unsupported.
...
The default replay depth is one direct premise level; `--depth N` exposes deeper existing ancestry without changing proof-search semantics.

Known special statement types are rendered mathematically where supported. Unsupported aggregate statements use a safe type-name fallback instead of leaking a Python dataclass representation.
...
Global help lists the user-facing commands and describes `n,k` using the project quantity $\pi_{n+k}^n$, i.e. free part plus the 2-primary component.
...
Phase 111 audited the existing CLI from the user's point of view before adding further capability.

Major outcomes:

```text
CLI capability / discoverability audit
→ global command help

command-overlap audit
→ preserve query / query-proof / show-proof / execute boundaries

operation-query grammar pressure audit
→ add only demonstrated flat three-term composition support

proof-replay usability audit
→ expose existing max_depth through --depth

full-report / replay presentation audit
→ fix symbolic-dimension rendering
→ prevent raw dataclass repr leakage in deep show-proof
```

The phase deliberately did not introduce:

- a general expression parser,
- arbitrary nested composition,
- Unicode composition parsing,
- general $E/H/\Delta$ evaluation,
- alternate-provenance selection,
- theorem ranking,
- broader qualified execution families.
...
Phase 111 closed the CLI capability / user-pressure audit and the minimal usability fixes justified by that audit.
