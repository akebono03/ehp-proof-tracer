...
def _step_derivation_sources_by_target_id(
  presentation: TodaGroupProofPresentation,
  blocks: tuple[
    TodaGroupProofNarrativeBlock,
    ...,
  ...
  for transition in transitions:
    ...
    existing = sources_by_target_id.get(
      target_id,
      (),
    )

    if any(
      source_step is transition.source_step
      for source_step in existing
    ):
      continue
  ...
  return sources_by_target_id


def _reorder_toda_group_proof_narrative_calculation_derivations(
  block: TodaGroupProofNarrativeBlock,
  ...
  for target_step in block.steps:
    ...
    for proof_step in block.steps:
      ...
      if proof_step_id in emitted_step_ids:
        continue

      ordered_steps.append(
        proof_step
      )
      emitted_step_ids.add(
        proof_step_id
      )
    ...
    if target_step_id not in emitted_step_ids:
      ordered_steps.append(
        target_step
      )
      emitted_step_ids.add(
...
def _insert_toda_group_proof_narrative_step_derivation_connectors(
  block_lines: tuple[
    str,
    ...,
  ],
