...
for node in ast.walk(tree):
  ...
  if not (
    ...
    and arg.value.attr == "membership"
    and isinstance(arg.value.value, ast.Name)
    and arg.value.value.id == "statement"
  ):
    continue
...
if len(targets) != 1:
  raise RuntimeError(
    ...
    "statement.membership.group with "
    "render_toda_expression_latex; "
    f"found {len(targets)}."
  )
...
print(
  "Phase 143-75AP R6 AST-targeted "
  "membership-group rendering repair applied."
)
