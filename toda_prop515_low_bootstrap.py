...
def _require_unique_inference_step(
  ...
  if len(unique_matches) != 1:
    raise RuntimeError(
      "expected exactly one distinct derived "
      f"{statement_type.__name__}; "
      f"found {len(unique_matches)} "
      f"distinct conclusions across "
      f"{len(matches)} matching proof steps"
    )
