...
# Phase 86・喘ounded-search depth parameterization

Phase 86 縺ｯ Phase 85 縺ｮ bounded depth=2 producer search / diagnostics / integrated execution 繧貞渕貅悶→縺励※縲・depth limit 縺ｮ譏守､ｺ蛹悶→蟆・擂縺ｮ bounded depth > 2 縺ｸ縺ｮ螳牙・縺ｪ荳闊ｬ蛹悶ｒ騾ｲ繧√ｋ infrastructure phase 縺ｨ縺吶ｋ縲・
譁ｰ縺励＞謨ｰ蟄ｦ theorem semantics 縺ｯ霑ｽ蜉縺励↑縺・・莉｣陦ｨ actual theorem 縺ｯ蠑輔″邯壹″ Toda Lemma 5.16 final bracket-sum consequence 繧剃ｽｿ逕ｨ縺吶ｋ縲・
## Phase 86-1・喇ard-coded depth=2 compatibility audit

迴ｾ陦後さ繝ｼ繝峨→髢｢騾｣繝・せ繝医ｒ逶｣譟ｻ縺励◆縲・
遒ｺ隱榊ｯｾ雎｡:
...
```text
BoundedProducerSearchNode / Result
竊・representation 閾ｪ菴薙・ depth=2 蝗ｺ螳壹〒縺ｯ縺ｪ縺・
select_unique_depth_two_producer_chain()
竊・direct / nested 縺ｮ2谿ｵ讒狗ｯ峨′ hard-coded

diagnose_depth_two_producer_search_failure()
竊・depth=2 / required next depth=3 縺・hard-coded

diagnose_depth_two_producer_execution_failure()
竊・selected producer_nodes 繧呈ｶ郁ｲｻ縺吶ｋ讒矩縺ｧ豈碑ｼ・噪荳闊ｬ逧・
build_depth_two_producer_search_report()
竊・depth-two search / diagnostic orchestration 繧貞他縺ｶ

execute_depth_two_producer_search()
竊・selected producer_nodes 繧・dependency-first 縺ｧ螳溯｡後☆繧九◆繧∵ｯ碑ｼ・噪荳闊ｬ逧・```

actual Toda Lemma 5.16 path:

```text
final
笏懌楳 bracket-sum              depths=(1,2)
笏披楳 composition              depths=(1,)
   笏披楳 bracket-sum
```
...
Phase 86-1 縺ｧ縺ｯ production code 繧貞､画峩縺帙★縲￣hase 86-2 縺ｮ譛蟆丞､画峩蠅・阜繧堤｢ｺ螳壹＠縺溘・
### 迥ｶ諷・
COMPLETE

---

## Phase 86-2・啼xplicit max_depth parameterization / max_depth=2 compatibility

逶ｮ逧・

```text
existing implicit depth=2 API
+
explicit max_depth=2
竊・exact same Phase 85 behavior
```
...
bool rejected
max_depth must equal 2 in Phase 86-2
```

縺励◆縺後▲縺ｦ Phase 86-2 縺ｧ縺ｯ:

```text
max_depth=1 竊・rejected
max_depth=2 竊・accepted
max_depth=3 竊・rejected
```

縺ｨ縺励￣hase 86-3 縺ｮ depth>2 capability 繧貞・蜿悶ｊ縺励↑縺・・
霑ｽ蜉 test:
...
default final rule
=
explicit max_depth=2 final rule

default selected producer path
=
explicit selected producer path

default node depths
=
explicit node depths
...
default goal conclusion
=
explicit goal conclusion

default inference-rule provenance
=
explicit inference-rule provenance
...
Phase 84窶・6-2 related regression:
...
## Phase 86-2A・嗷epresentative probe + completion documentation
...
same final rule = True
same producer path = True
same depths = True
same dependencies = True

default producer node count = 2
explicit producer node count = 2

default shared depths = (1, 2)
explicit shared depths = (1, 2)

default shared dependency = True
explicit shared dependency = True

default goal derived = True
explicit goal derived = True

same goal conclusion = True
same goal inference rule = True
same goal premise conclusions = True
same goal premise rules = True

diagnostic status = depth_limit
current depth = 2
...
Phase 85 behavior preserved:

```text
default call
=
explicit max_depth=2 call
```

Phase 86-2 縺ｧ縺ｯ諢丞峙逧・↓譛ｪ螳溯｣・

```text
max_depth > 2 execution
...
Phase 86-3 縺ｮ譛蛻昴・譛蟆・target:

```text
unique depth=3 producer chain

max_depth=2
竊・DEPTH_LIMIT

max_depth=3
竊・selected dependency DAG
竊・dependency-first execution
竊・goal ProofStep
```
...
---
## 2026-09-16 Phase 86-4：bounded depth=4 completion

Phase 86-4-1 で `max_depth > 3` compatibility / hard-coded depth audit を行い、selection / diagnostics / report / execution の内部構造は depth=3 固定ではなく、実際の blocker が validator の `max_depth in (2, 3)` であることを確認した。

Phase 86-4-2 で validator を `integer max_depth >= 2` に一般化し、depth=4 fixture `D -> C -> B -> A -> final -> goal` を追加した。`max_depth=3` では `DEPTH_LIMIT`, `current_depth=3`, `required_next_depth=4` を返す。

Phase 86-4-3 で `max_depth=4` selection を正式検証し、dependency-first order `D, C, B, A` と depths `((4,), (3,), (2,), (1,))` を固定した。

Phase 86-4-4 で diagnostics / report integration を検証し、成功 chain では `diagnostic=None`, `report.status=SUCCESS`, `report.search_result.max_depth=4` を確認した。

Phase 86-4-5 で selected-path execution を検証した。
...
Phase 86-4-6 では representative probe に depth=4 boundary pair と execution provenance を追加し、Phase 86-4 completion documentation を固定する。
...
次は Phase 87 の producer ambiguity / explicit finite retry policy audit。

---

# Phase 87：finite producer retry

Phase 86 までの bounded producer search は:

```text
複数 safe producer candidate
-> AMBIGUOUS_PRODUCER
-> stop
```

という conservative policy だった。

Phase 87 では general backtracking を導入せず、明示的な有限 retry policy のみを追加した。

## Phase 87-1：ambiguity semantics / retry compatibility audit
...
unique-safe-producer compatibility
final-rule ambiguity boundary
```
...
retry policy あり
-> safe candidates を有限個だけ試す
```

general backtracking / ranking / cost model は対象外。
...
## Phase 87-2：minimal retry policy representation / fixture
...
Phase 87-2 時点では search semantics を変更せず、複数候補は従来どおり ambiguity。

確認:

```text
14 passed
全体 7016 passed in 42.31s
```
...
## Phase 87-3：selection-side finite retry

`select_unique_depth_two_producer_chain()` に:
...
## Phase 87-4：diagnostics / report integration
...
PRODUCER_RETRY_EXHAUSTED
= 明示 retry policy の試行上限内で成功 candidate が得られなかった
```

`diagnose_depth_two_producer_search_failure()` と `build_depth_two_producer_search_report()` に retry policy を統合。

確認:

```text
102 focused passed
全体 7034 passed in 40.75s
```
...
## Phase 87-5：selected-path execution with retry provenance

`execute_depth_two_producer_search()` に retry policy を伝播。

重要境界:

```text
execution layer は producer candidate を再探索しない
```
...
-> selected producer ProofStep
-> final ProofStep
-> goal derived
```

failed first candidate と、その dependency branch は execution provenance に現れない。

確認:

```text
90 focused passed
全体 7043 passed in 41.53s
```
...
## Phase 87-6：representative probe / documentation / completion
...
failed first candidate not executed
failed dependency branch not executed
repository non-mutation
```

Phase 87 completion boundary:

```text
finite explicit retry policy
selection-side retry
