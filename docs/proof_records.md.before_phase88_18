...
# 37. Phase 86 explicit max_depth compatibility infrastructure record
...
Phase 86-2 縺ｯ譁ｰ縺励＞謨ｰ蟄ｦ theorem 繧定ｿｽ蜉縺励↑縺・・
譌｢蟄倥・ Toda Lemma 5.16 final bracket-sum consequence 繧・actual theorem 縺ｨ縺励※蜀榊茜逕ｨ縺励・Phase 85 depth=2 bounded-search semantics 縺ｨ explicit `max_depth=2` 縺ｮ螳悟・莠呈鋤諤ｧ繧定ｨ倬鹸縺吶ｋ縲・
謨ｰ蟄ｦ逧・formal proof record 謨ｰ縺ｯ13縺ｮ縺ｾ縺ｾ縲・
---
...
```text
implicit depth=2 bounded producer search
竊・diagnostics
竊・selected-path execution
竊・goal ProofStep
```
...
## 37.3 representative actual theorem
...
Thus explicit parameterization does not change the selected proof plan.

---

## 37.5 shared dependency record

Both default and explicit `max_depth=2` preserve:
...
The same logical shared-dependency DAG remains the Phase 85 baseline.

---

## 37.6 execution / provenance record
...
The parameterization therefore preserves the existing Toda Lemma 5.16 `ProofStep` provenance structure.

---

## 37.7 diagnostic compatibility

Representative depth-limit diagnostic:
...
The Phase 85 diagnostic boundary is preserved under explicit `max_depth=2`.

---

## 37.8 safety / phase boundary
...
Non-integer / bool misuse is also rejected by validation.

This is deliberate: Phase 86-2 parameterizes the existing depth=2 behavior without implementing depth>2 search.

---

## 37.9 representative probe
...
## 37.10 regression record

Phase 86-2 parameterization focused:
...
Phase 86-2 + representative probe:

```text
24 passed in 2.18s
```

repository-wide after probe:
...
## 37.11 completion boundary

Phase 86-2 is COMPLETE.
...
DFS / BFS / A*
arbitrary recursive theorem search
```
...
The first intended test pair is:

```text
same unique depth=3 dependency
...
# 38. 迴ｾ蝨ｨ縺ｮ proof-record 迥ｶ諷・after Phase 86-2
...
---
# Phase 86-4 representative proof-search record

この記録は数学定理そのものではなく、bounded proof-search infrastructure の代表 trace を記録する。
...
Selected producer nodes:

```text
dependency-first order = D, C, B, A
depths = ((4,), (3,), (2,), (1,))
```
...
```text
D ProofStep: premises=()
C ProofStep: premises=(D ProofStep,)
B ProofStep: premises=(C ProofStep,)
A ProofStep: premises=(B ProofStep,)
goal ProofStep: premises=(A ProofStep,)
```

最終 goal conclusion は requested goal と一致し、元 repository は mutate しない。

formal regression 済み:
